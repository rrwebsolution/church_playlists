...
export default function App() {
  ...
  // --- SYNC PLAYER CONTENT (FIXED CRASH) ---
  useEffect(() => {
    if (ytPlayerRef.current && activeVideoId && isPlaying) {
      // GI-FIX: Naggamit na og optional chaining aron dili mo crash kung undefined ang videoData
      const videoData = ytPlayerRef.current.getVideoData?.();
      const internalId = videoData?.video_id; 

      if (internalId && internalId !== activeVideoId) {
  ...
  return (
    <div className="flex h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 overflow-hidden relative">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
        <Header activeMenu={currentActiveMenu} activeFolderId={activeFolderId} inputValue={inputValue} setInputValue={setInputValue} onSubmit={handleHeaderSubmit} setIsSidebarOpen={setIsSidebarOpen} isFetching={isFetching} searchMode={searchMode} setSearchMode={setSearchMode} bgPlayEnabled={bgPlayEnabled} setBgPlayEnabled={setBgPlayEnabled} />
            ...
            <div className="bg-zinc-900 rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl border border-white/10 group relative">
              ...
              <div className="aspect-video relative bg-black">
                <YouTube
                  ...
                  opts={{ width: '100%', height: '100%', playerVars: { autoplay: 1, mute: 1, playsinline: 1, controls: 1, rel: 0, origin: window.location.protocol + '//' + window.location.host } }}
                  onReady={(e) => { 
