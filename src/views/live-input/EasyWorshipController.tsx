...
export default function EasyWorshipController() {
  ...
  const broadcastData = async (text: string, size: number, bg: string) => {
    ...
    const obsUrl = import.meta.env.VITE_OBS_STATE_URL || 'api/obs-state';
    fetch(obsUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).catch(() => {});
